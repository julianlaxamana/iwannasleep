...
class MultiCameraViewer(QWidget):
    def __init__(self):
        ...
        self.toggle_buttons = [QPushButton("Turn Off") for _ in range(3)]
        self.selected_camera_indices = [0, 1, 2]  # Default mappings
        self.feed_enabled = [True] * 3  # All feeds enabled by default
    ...
    def toggle_feed(self, index, checked):
        self.feed_enabled[index] = not checked
        self.toggle_buttons[index].setText("Turn On" if checked else "Turn Off")
        if not self.feed_enabled[index]:
